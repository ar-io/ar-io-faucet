...
import { generateToken, transferToken, verifyTokenAndExpiration  } from './lib/crypto.js';
...
router.post('/api/mint', async (ctx) => {
    ...
    try {
        const txId = await transferToken(token);
        ctx.body = { id: txId, status: 'success' };
    } catch (error: any) {
        ctx.status = 503;
        ctx.body = { error: 'Failed to mint token', message: error.message, stack: error.stack };
    }
}); 
