...
export const generateToken = async (recipient: string): Promise<string> => {
    ...
    const signature = await arweave.crypto.sign(WALLET, Buffer.from(payloadString));
 ...
 */
export const verifyTokenAndExpiration = async (token: string): Promise<{ isValid: boolean, payload: any }> => {
    const tokenData = JSON.parse(Buffer.from(token, 'base64').toString('utf8'));
    const { payload: payloadString, signature } = tokenData;

    const payload = JSON.parse(payloadString);
    const isValid = await arweave.crypto.verify(WALLET.n, Buffer.from(payloadString), Buffer.from(signature, 'base64'));
 ...
 */
export const transferToken = async (token: string): Promise<string> => {
    const { isValid, payload } = await verifyTokenAndExpiration(token);
    if (!isValid) {
        throw new Error('Invalid token');
    }

    const { recipient, qty } = payload;
