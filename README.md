...
```bash
curl -X POST http://localhost:3000/api/request -H "Content-Type: application/json" -d '{"recipient": "recipient_address"}'
```
...
```bash
curl -X POST http://localhost:3000/api/mint -H "Content-Type: application/json" -d '{"token": "token"}'
```

## Verification

Users can verify a token by sending a POST request to the `/api/verify` endpoint with the token in the request body.

```bash
curl -X POST http://localhost:3000/api/verify -H "Content-Type: application/json" -d '{"token": "token"}'
```
